...
allPoints=[[0 for i in range(10000)] for j in range(10000)]
...
class DataReceiverThread(QThread):
    ...
    def __init__(self, points ,robot_x ,robot_y, robot_angle):
        ...
        self.ipAddress = "192.168.71.221"
        self.remote_port = 8655
        print("girdi3")
                        ...
                        if not bytesRead:
                            ...
                            for i in range(point_count):
                                ...
                                with QMutexLocker(self.mutex):
                                    ...
                                    self.points.append([point[0]+robotX  , point[1] +robotY])
                                    
                                    allPoints[int(1000*point[0])][int(1000*point[1])]=1
                                    print("point x:",point[0],"point y:",point[1] ,allPoints[int(point[0])][int(point[1])])
                                    #  print("Point x : " , float(point[0]) + self.robot_x, "Point y : " , float(point[1]) + self.robot_y)

                            with QMutexLocker(self.mutex):
                                while len(self.points) > 10000:
                                    self.points.pop(0)
...
# Convert the points to a list of QPoint objects

        #transformed_points = [
        #	QPoint(int((point[0] * self.zoom) + self.camera_x), int((point[1] * self.zoom) + self.camera_y))
        #	for point in self.points
        #]
        painter.setPen(QPen(Qt.red, 3))
        for row in range(10000):
            for col in range(10000):
                if(allPoints[row][col]==1):
                    print(row,col,"1 oldu")
                    painter.drawPoint(QPoint(int((row* self.zoom) + self.camera_x), int((col * self.zoom) + self.camera_y)))


# Set the painter's pen
        

		# Draw all points using drawPoints method
        #painter.drawPoints(transformed_points)
...
"background:  transparent;} ")
        ...
        self.Button_Calibration.setGeometry(QtCore.QRect(645, 160, 90, 30))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setBold(False)
        font.setWeight(50)
